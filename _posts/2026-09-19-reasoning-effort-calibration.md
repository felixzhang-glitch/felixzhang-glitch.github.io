...
1. **档位是白名单，不是区间。** 传入模型不支持的取值直接返回 `invalid_parameter_error`，部分模型甚至要求完全匹配，不做任何兜底
2. **默认档普遍偏高。** Qwen3.8 默认 `xhigh`、GLM-5.3 默认 `max`、DeepSeek 默认 `high`。不显式指定，账单按最贵的那档走
3. **同名不同义。** `high` 在 Qwen3.8 上等价于 `xhigh`，在 DeepSeek 快照版上是真实的高档，在 GLM-5.2 上是"三档合一"的汇聚点
4. **DeepSeek 是唯一把 effort 做成训练期连续标量的厂商。** API 暴露的三档只是 1–100 标量上的三个采样点
5. **参数正在被两层结构取代：** 上层是 `effort` 粗调旋钮，下层是 `thinking_budget` / `task_budget` 硬预算。两者关系在每家都不一样
...
- **qwen3.8 系列不支持 `reasoning_effort` 与 `thinking_budget` 同时出现**，同时传会报错。两者可互转：`low` = 4096、`medium` = 16384、`xhigh` = 262144；都不传时默认 `thinking_budget` 131072，即 `xhigh`
...
第 3 条值得单独说：百炼的 GLM 页面自带一条警告——云上部署的三方开源模型与模型官方对超参数的处理逻辑不同，**官方做阈值校验、越界回退默认值；云上是直接透传，不做校验**。这意味着 `glm-5.2` 在百炼上更接近"原始透传"，而 `ZHIPU/GLM-5.3` 是智谱直供、走官方校验。同一平台内，两类模型的容错行为是反的。
...
`reasoning_effort` 是企业把算力-质量权衡外包给调用方的产物：参数是统一的，语义是分裂的，选档位的责任和成本一起转移给了 API 使用者。工程上唯一可靠的做法是——按模型查表，别信直觉。